  ...
  return (
    <div className="block sticky top-0 bg-white dark:bg-neutral-900 dark:text-white">
      <div className="m-5 ml-10 mr-10 flex items-center justify-between p-2">
        <Link className={`text-lg ${isActiveLink("/")}`} to={`/`}>
          ksskol
        </Link>
        ...
        <div className="hidden laptop:flex items-center space-x-5 ">
          <Link to={`/about`} onClick={() => handleLinkClick("/about")} className={`p-2 ${isActiveLink("/about")}`}>
            about
          </Link>
          <Link to={`/projects`} onClick={() => handleLinkClick("/projects")} className={`p-2 ${isActiveLink("/projects")}`}>
            projects
          </Link>
          <Link to={`/resume`} onClick={() => handleLinkClick("/resume")} className={`p-2 ${isActiveLink("/resume")}`}>
            resume
          </Link>
